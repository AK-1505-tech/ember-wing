...
function MiniApp() {
  ...
  useEffect(() => {
    // Determine Ollama endpoint based on environment
    // In local dev: use localhost:11434
    // In Netlify: use /.netlify/functions/chat (which proxies to your Ollama server)
    if (process.env.NODE_ENV === 'development') {
      aiRef.current = 'http://localhost:11434'; // local Ollama for development
    } else {
      // Production: use Netlify Function as proxy
      // The function forwards to OLLAMA_URL (set in Netlify env vars)
      aiRef.current = '/.netlify/functions/chat';
    }
    console.log('AI endpoint:', aiRef.current);
    console.warn('Make sure Ollama is running: ollama pull gemma3:4b && ollama serve');

    // Handle hash changes to toggle .main visibility
    const handleHashChange = () => {
      const main = document.querySelector('.main');
      if (main) {
        // Hide main if there's a hash targeting a dialog
        if (window.location.hash === '#about-me' || window.location.hash === '#ai-thinking') {
  ...
  return (
    <>
      <link href="https://ronenness.github.io/RPGUI/rpgui/rpgui.min.css" rel="stylesheet" type="text/css" />
      <script src="https://ronenness.github.io/RPGUI/rpgui/rpgui.min.js"></script>
      <link href='https://unpkg.com/7.css' rel='stylesheet' />
      <div className='rpgui-content'>
        <div className='rpgui-container framed' style={{ width: '100vw', height: '100vh' }}>
          <div className='main'>
            <ul role='menubar' className='can-hover' style={{ fontSize: '2.5vw', zIndex: 1001, position: 'relative' }}>
            ...
            <div className='window-body has-space rpgui-container framed-golden' style={{ height: 'calc(100vh - 160px)',width: 'calc(90vw - 26.4px)' , marginTop: '1vh', display: 'flex', flexDirection: 'column' }}>
              <div className='' style={{ display: 'flex', flexDirection: 'column', flex: 1, overflowY: 'auto', paddingRight: '8px', marginBottom: '10px' }}>
                {
              ...
              <div className="group " style={{ position: 'fixed', bottom: '30px', width: '84vw', left: '35px', zIndex: 1000 }}>
                <label htmlFor="UserIn">Type your message: </label>
                <input id="UserIn" type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={askAI} disabled={isThinking || retryCountdown > 0 || cooldown > 0} style={{ width: '100%', zIndex: 1000 }}></input>
          ...
          <div className='rpgui-content'>
            <div className="rpgui-container framed window" id="ai-thinking" role="dialog" aria-labelledby="dialog-title" style={{ visibility: isThinking ? 'visible' : 'hidden', opacity: isThinking ? '1' : '0', zIndex: 1002 }}>
            ...
            <div style={{ width: '100vw', height: '100vh', position: 'absolute', backgroundColor: 'rgba(0,0,0,0.4)', top: 0, left: 0, display: isThinking ? 'block' : 'none', zIndex: 999 }}></div>
            <div className='rpgui-container framed window' id='about-me' role='dialog' aria-labelledby='dialog-title' style={{ width: '100vw', height: '100vh', '--w7-w-bg': '#0fa' }}>
              <div className='window-body has-space' style={{ backgroundColor: '#0000', border: 'none' }}>
                <h2 className='instruction instruction-primary'>About</h2>
              ...
              <footer style={{ textAlign: 'right', backgroundColor: '#0000', border: 'none' }}>
                <button onClick={() => navigate(-1)} className='rpgui-button' type='button'>Okay!!!</button>
              </footer>
            </div>
          </div>
        </div>
